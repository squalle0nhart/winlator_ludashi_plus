...
bool ASurfaceRendererContext::loadScanoutApi() {
    ...
    bool coreOk = fnSCCreateFromWin && fnSCRelease && fnSTCreate && fnSTDelete &&
            fnSTSetOnComplete && fnSTReparent && fnSTApply && fnSTSetBuffer &&
            fnSTSetVisibility && fnSTSetGeometry && fnSTSetBufferTransparency &&
            fnSTSetBufferTransform;
    if (!coreOk) {
...
void ASurfaceRendererContext::destroyScanout() {
    ...
    if (scanoutCursorBuf) {
        if (scanoutCursorFence >= 0) {
            close(scanoutCursorFence);
            scanoutCursorFence = -1;
        }
...
void ASurfaceRendererContext::scanoutSetCursorImage(void* pixels, short w, short h, short stride) {
    ...
    void* dst = nullptr;
    if (AHardwareBuffer_lock(scanoutCursorBuf, AHARDWAREBUFFER_USAGE_CPU_WRITE_OFTEN, -1,
                             nullptr, &dst) != 0) {
        return;
    }
...
void ASurfaceRendererContext::scanoutSetCursorPos(short x, short y, short hotX, short hotY, bool cursorVisible) {
    if (!scanoutActive.load() || !scanoutCursorSC || scanoutCursorBufW <= 0 || scanoutCursorBufH <= 0) return;
    if (x == lastRawCursorX && y == lastRawCursorY && hotX == lastRawHotX && hotY == lastRawHotY) return;
    lastRawCursorX = x;
    lastRawCursorY = y;
    lastRawHotX = hotX;
    lastRawHotY = hotY;
...
void ASurfaceRendererContext::setWindowBuffer(int64_t contentId, AHardwareBuffer* ahb,
                                              int fenceFd, int64_t windowId, int64_t serial) {
    if (!ahb) {
    ...
    if (windowId != 0 || serial != 0) attachOnCompleteCallback(tx, windowId, serial);
    ST_APPLY(tx);
    ST_DELETE(tx);
}
...
void ASurfaceRendererContext::scanoutFrameCompleteCallback(void* ctxPtr, void* stats) {
    ...
    if (target && target->globalRef && target->methodId) {
        ...
        if (env) {
            int64_t packed = ((int64_t) (uint32_t) pCtx->windowId << 32) |
                    ((int64_t) (uint32_t) pCtx->serial);
...
void ASurfaceRendererContext::beginTransaction() {
    if (!currentTx) currentTx = ST_CREATE();
}
...
void ASurfaceRendererContext::updateWindow(int64_t contentId, bool visible, int zOrder,
    ...
    if (visible) {
        ARect localSrcR{srcL, srcT, srcR, srcB};
        ARect localDstR{dstL, dstT, dstR, dstB};
        ST_SETGEO(tx, sc, &localSrcR, &localDstR, 0);
...
void ASurfaceRendererContext::registerWindowSC(int64_t contentId, const char* debugName) {
    ...
    void* sc = SC_CREATE(window, debugName ? debugName : "(x11_window)");
    if (!sc) return;
