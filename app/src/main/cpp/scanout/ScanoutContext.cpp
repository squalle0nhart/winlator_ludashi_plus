...
#define SCO_RLOG(...) do { if (verboseLog) \
    __android_log_print(ANDROID_LOG_DEBUG, "Winlator_Renderer", __VA_ARGS__); } while (0)
...
bool ScanoutContext::loadApi() {
    ...
    if (!lib) {
        SCANOUT_LOG("loadScanoutApi: dlopen failed: %s", dlerror());
        return false;
    }
...
void ScanoutContext::initFromWindow(ANativeWindow* win) {
    if (win) window = win;
    ...
    if (!scanoutGameSC || !scanoutCursorSC) {
        ...
        if (scanoutGameSC) {
            SC_RELEASE(scanoutGameSC);
            scanoutGameSC = nullptr;
        }
        if (scanoutCursorSC) {
    ...
    ST_SETZORDER(setupTx, scanoutGameSC, 0);
    ST_SETVIS(setupTx, scanoutGameSC, 0);
    ST_SETZORDER(setupTx, scanoutCursorSC, 1);
    ST_SETVIS(setupTx, scanoutCursorSC, 0);
    ST_APPLY(setupTx);
    ST_DELETE(setupTx);
...
void ScanoutContext::initFromWindows(ANativeWindow* gameWin, ANativeWindow* cursorWin) {
    if (scanoutActive.load()) destroy();
    if (!loadApi()) {
        ANativeWindow_release(gameWin);
        ANativeWindow_release(cursorWin);
        initFromWindow();
        return;
    }
...
void ScanoutContext::destroy() {
    ...
    if (scanoutGameTx) {
        ST_DELETE(scanoutGameTx);
        scanoutGameTx = nullptr;
    }
    if (scanoutTx) {
        ST_DELETE(scanoutTx);
        scanoutTx = nullptr;
...
void ScanoutContext::applyPendingCursor() {
    bool hasImage = false;
    bool hasPos = false;
    short cx = 0;
    short cy = 0;
    ...
    {
        std::lock_guard<std::mutex> lk(scanoutMutex);
        hasImage = cursorImageDirty;
        cursorImageDirty = false;
        hasPos = cursorPosDirty;
        cursorPosDirty = false;
        cx = pendingCursorX;
        cy = pendingCursorY;
        chx = pendingCursorHotX;
        chy = pendingCursorHotY;
    }
...
void ScanoutContext::setDst(int x, int y, int w, int h) {
    scanoutDstX = x;
    scanoutDstY = y;
    scanoutDstW = w;
    scanoutDstH = h;
...
bool ScanoutContext::setCursorImage(void* pixels, short w, short h, short stride) {
    if (!scanoutActive.load() || !scanoutCursorSC || !pixels || w <= 0 || h <= 0) return false;
    ...
    if (!*curBuf) {
        ...
        if (AHardwareBuffer_allocate(&d, curBuf) != 0) return false;
        scanoutCursorBufW = w;
        scanoutCursorBufH = h;
    }
...
bool ScanoutContext::setCursorPos(short x, short y, short hotX, short hotY) {
    if (!scanoutActive.load() || !scanoutCursorSC) return false;
    if (scanoutCursorBufW <= 0 || scanoutCursorBufH <= 0) return false;
    {
        std::lock_guard<std::mutex> lk(scanoutMutex);
        pendingCursorX = x;
        pendingCursorY = y;
        pendingCursorHotX = hotX;
        pendingCursorHotY = hotY;
        cursorPosDirty = true;
    }
    return true;
}
