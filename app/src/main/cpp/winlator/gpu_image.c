...
Java_com_winlator_cmod_renderer_GPUImage_hardwareBufferFromSocket(JNIEnv *env, jobject obj, jint fd) {
    AHardwareBuffer *ahb;
    uint8_t buf = 1;
    ...
    if (cls) {
        ...
        jfieldID fFormat = (*env)->GetFieldID(env, cls, "format", "I");
        if (fFormat)
            (*env)->SetIntField(env, obj, fFormat, (jint)desc.format);
    }
...
JNIEXPORT jlong JNICALL
Java_com_winlator_cmod_renderer_GPUImage_createHardwareBuffer(JNIEnv *env, jclass obj,
...
Java_com_winlator_cmod_renderer_GPUImage_lockHardwareBuffer(JNIEnv *env, jobject obj, jlong ptr) {
    ...
    jobject buffer = (*env)->NewDirectByteBuffer(env, addr,
