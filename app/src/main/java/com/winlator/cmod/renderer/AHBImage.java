...
/**
 * Fenced, CPU-writable AHardwareBuffer texture used by the SurfaceFlinger (ASR) renderer for
 * CPU-drawn window chrome. Ported from GameNative PR #1620 (the AHBImage/GPUImage split).
 *
 * <p>Unlike {@link GPUImage} (the simple GL/Vulkan/DRI3/Present texture), AHBImage owns a stable
 * three-buffer scanout swapchain and per-slot release fences. {@code getScanoutHardwareBufferPtr()}
 * copies the freshly drawn CPU pixels into the next swapchain slot (honouring the slot's release
 * fence) and hands the AHB to SurfaceFlinger with an acquire fence — this is what fixes the
 * SurfaceFlinger tearing/crash on CPU images. The buffers are allocated BGRA; the ASR native GPU
 * converter turns them into RGBA when colour-compat mode is enabled.</p>
 *
 * <p>Native methods live in {@code libahbimage.so} ({@code cpp/asurfacerenderer/ahbimage.c}); the
 * three static scanout hooks are served by {@code libasurface_renderer.so} via
 * {@link ASurfaceRenderer}.</p>
 */
