        ...
        for (final int port : gamepadClients) {
            addAction(() -> {
                ...
                if (enabled) {
                    ...
                    if (gyroToLeftStick) {
                        state.thumbLX = Mathf.clamp(state.thumbLX + gyroX, -1.0f, 1.0f);
                        state.thumbLY = Mathf.clamp(state.thumbLY + gyroY, -1.0f, 1.0f);
                    } else {
...
//    }
    ...
    public void setGyroTriggerButton(int keyCode) {
        this.gyroTriggerButton = keyCode;
    }

    public void setGyroToggleMode(boolean toggleMode) {
        this.isToggleMode = toggleMode;
    }

    public void setGyroToLeftStick(boolean toLeftStick) {
        this.gyroToLeftStick = toLeftStick;
    }

    public void reloadTurboFromPrefs() {
        // Compatibility shim for dialogs that request live turbo refresh.
    }
